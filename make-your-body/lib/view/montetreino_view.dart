...
class _MonteTreinoViewState extends State<MonteTreinoView> {
  ...
  void _salvarTreino(String nome) async {
    ...
    debugPrint(
      'Salvando treino "$nome" com ${exerciciosSelecionados.length} exercicios',
    );

    // TODO: Implementar salvamento quando Firebase estiver configurado
    // final meusTreinosController = GetIt.instance.get<MeusTreinosController>();
    // await meusTreinosController.adicionarTreino(nome, exerciciosSelecionados);
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          'Treino "$nome" criado! (Salvamento temporariamente desabilitado)',
        ),
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Treino "$nome" criado com sucesso!'),
        backgroundColor: Colors.green,
